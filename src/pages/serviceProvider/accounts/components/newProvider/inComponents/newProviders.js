...
class ServiceProvider extends Component {
  ...
  componentDidMount() {
    ServiceProviderService.fetchServiceProvider().then(res => {
      console.log(" server response", res);
      if(res.data){
        this.setState({dataSource: res.data.content, isLoading: false  });
        message.success('Service Provider details fetched succesfully')
      } else {
        message.error('Theres a problem fetching Sp details. Try refreshing the page')
      }
    });
  }
  ...
  handlecConfirms = (row) => {
    ...
    ServiceProviderService.deleteMemberVehicleType(row.type_id)
  ...
  render() {
    ...
    return (
      <Card type={"inner"} title={"Service Provider"}>
        <Table
          loading={isLoading}
